...
/// Helper to create a dummy executable for validation tests
fn create_dummy_exec() -> (PathBuf, tempfile::TempDir) {
    let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
    let exec_path = temp_dir.path().join("firecracker-dummy");
    File::create(&exec_path).expect("Failed to create dummy exec");
...
fn test_verify_jailer_installed() {
    use crate::vm::jailer::verify_jailer_installed;
...
fn test_jailer_config_valid_id() {
    let (exec_path, _temp) = create_dummy_exec();
    let mut config = JailerConfig::new("test-vm-123".to_string());
    config.exec_file = exec_path;
...
fn test_jailer_config_empty_id() {
    ...
    // Should fail regardless of exec path because ID is invalid
...
fn test_property_valid_ids_pass() {
    ...
    for id in valid_ids {
        ...
        config.exec_file = exec_path.clone();
        assert!(
...
fn test_property_invalid_ids_fail() {
    ...
    for id in invalid_ids {
        ...
        // Should fail regardless of exec file
        assert!(
