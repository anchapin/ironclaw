...
#[cfg(test)]
mod tests {
    use crate::vm::{destroy_vm, spawn_vm};
    #[cfg(unix)]
    ...
    async fn test_vsock_paths_are_unique() {
        ...
        // Paths contain hash, so original ID won't be visible
    ...
    async fn test_vm_with_long_id() {
        ...
        assert_eq!(handle.id.len(), 100);

        #[cfg(unix)]
        {
            // Verify firewall chain name is valid (hashed so short)
    ...
    async fn test_vm_with_special_chars() {
        ...
        // Path contains hash, not ID
        assert!(vsock_path.starts_with("/tmp/ironclaw/vsock/"));
        assert!(vsock_path.ends_with(".sock"));
