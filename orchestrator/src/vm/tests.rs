...
mod tests {
    ...
    async fn ensure_test_resources() -> Option<(String, String)> {
        ...
        let temp_dir = std::env::temp_dir().join("ironclaw-test-resources");
        let _ = tokio::fs::create_dir_all(&temp_dir).await;
        let kernel_path = temp_dir.join("vmlinux");
        let rootfs_path = temp_dir.join("rootfs.ext4");

        if !kernel_path.exists() {
            let _ = tokio::fs::File::create(&kernel_path).await;
        }
        if !rootfs_path.exists() {
            let _ = tokio::fs::File::create(&rootfs_path).await;
        }
    ...
    async fn spawn_test_vm(vm_id: &str) -> Option<crate::vm::VmHandle> {
        ...
        match spawn_vm_with_config(vm_id, &config).await {
    ...
    async fn test_multiple_vms_isolation() {
        let handle1 = match spawn_test_vm("task-1").await {
            Some(h) => h,
            None => {
                println!("Skipping test: Firecracker or resources not available");
                return;
            }
        };

        let handle2 = match spawn_test_vm("task-2").await {
            Some(h) => h,
            None => {
    ...
    async fn test_firewall_verification() {
        let handle = match spawn_test_vm("firewall-test").await {
            Some(h) => h,
            None => return,
        };
    ...
    async fn test_vsock_paths_are_unique() {
        let handle1 = match spawn_test_vm("vsock-unique-1").await {
            Some(h) => h,
            None => return,
        };
        let handle2 = match spawn_test_vm("vsock-unique-2").await {
            Some(h) => h,
            None => {
                destroy_vm(handle1).await.unwrap();
                return;
    ...
    async fn test_vm_with_long_id() {
        ...
        let handle = match spawn_test_vm(&long_id).await {
            Some(h) => h,
            None => return,
        };
    ...
    async fn test_vm_with_special_chars() {
        ...
        let handle = match spawn_test_vm(special_id).await {
            Some(h) => h,
            None => return,
        };
    ...
    async fn test_vm_cleanup_on_destruction() {
        let handle = match spawn_test_vm("cleanup-test").await {
            Some(h) => h,
            None => return,
        };
    ...
    async fn test_rapid_vm_lifecycle() {
        ...
        for i in 0..10 {
            if let Some(handle) = spawn_test_vm(&format!("rapid-{}", i)).await {
                assert!(handle.vsock_path().is_some());
                destroy_vm(handle).await.unwrap();
            } else {
                println!("Skipping rapid lifecycle test loop {}", i);
                return;
            }
        }
