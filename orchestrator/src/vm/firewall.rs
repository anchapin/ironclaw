...
impl FirewallManager {
    ...
    pub fn new(vm_id: String) -> Self {
        ...
        // Use FNV-1a hash to generate a short, unique suffix
        // This prevents collisions (e.g., vm-1 vs vm_1) and ensures
        // the chain name stays within iptables limits (28 chars).
        let hash = crate::vm::fnv1a_hash(&vm_id);
...
mod tests {
    ...
    fn test_firewall_manager_creation() {
        ...
        // Hash-based name won't contain the original ID
    }

    #[test]
    fn test_firewall_manager_collision_resistance() {
        // Test that similar IDs produce different chain names
        let id1 = "test-vm";
        let id2 = "test_vm"; // Would collide with simple sanitization

        let m1 = FirewallManager::new(id1.to_string());
        let m2 = FirewallManager::new(id2.to_string());

        assert_ne!(m1.chain_name(), m2.chain_name());

        // Ensure special chars are handled safely
        let m3 = FirewallManager::new("test-vm@123".to_string());
        assert!(m3.chain_name().starts_with("IRONCLAW_"));
        assert!(!m3.chain_name().contains('@'));
    }
